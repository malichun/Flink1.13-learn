...
package org.apache.flink_test.streaming.runtime.operators;
...
public class TimestampsAndWatermarksOperator<T> extends AbstractStreamOperator<T>
    ...
    public void processElement(final StreamRecord<T> element) throws Exception {
        ...
        System.out.println("watermark算子输出, 接收到一条用户数据, 并已经输出: "+ element.getValue());
        watermarkGenerator.onEvent(event, newTimestamp, wmOutput);
        System.out.println("watermark算子,根据收到的数据, 修改了maxTimestamp变量为: "+ newTimestamp);
    }

    @Override
    public void onProcessingTime(long timestamp) throws Exception {
        System.out.println("watermark算子的定时器被触发, 准备去调用watermark生成器的周期发射方法: ");
