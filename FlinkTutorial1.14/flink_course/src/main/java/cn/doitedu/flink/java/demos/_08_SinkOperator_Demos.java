...
public class _08_SinkOperator_Demos {
    public static void main(String[] args) throws Exception {
        ...
        streamSource.map(e -> Tuple4.of(e.getEventId(), e.getGuid(), e.getSessionId(), e.getEventInfo()))
            .returns(new TypeHint<Tuple4<String, Long, String, Map<String, String>>>() {
            })
        /* .writeAsCsv("d:/sink_test", FileSystem.WriteMode.OVERWRITE) */;

        // TODO 应用StreamFileSink 算子来将数据输出到文件系统
        ...
        // 1.构造一个fileSink
        // 构造一个FileSink对象
        FileSink<String> rowSink = FileSink.forRowFormat(new Path("file:///d:/filesink/"), new SimpleStringEncoder<String>("UTF-8"))
            // 文件的滚动策略 (间隔时长10s, 或者文件大小达到1M, 就进行文件切换)
            .withRollingPolicy(DefaultRollingPolicy.builder().withInactivityInterval(10_000).withMaxPartSize(1024 * 1024).build())
            // 分桶的策略, 划分子文件夹的策略, 在父目录下生成子目录
        ...
        //.addSink(// sinkFunction实现类对象) /* SinkFunction的实现类对象, 用addSink()来添加 */
        /* .sinkTo(rowSink) */; /* Sink的实现类, 用sinkTo来添加 */
