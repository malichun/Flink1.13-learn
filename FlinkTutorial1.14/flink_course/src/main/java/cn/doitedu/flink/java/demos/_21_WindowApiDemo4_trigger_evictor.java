 ...
 * @author malichun
 * @create 2022/09/09 0009 1:06
 */
public class _21_WindowApiDemo4_trigger_evictor {
...
/**
 * 拷贝EventTimeTrigger,修改
 * @see  org.apache.flink.streaming.api.windowing.triggers.EventTimeTrigger;
 */
class MyEventTimeTrigger extends Trigger<Tuple2<EventBean2, Integer>, TimeWindow> {
    ...
    /**
     * 来一条数据需要检查 watermark是否已经越过窗口结束点, 需要触发
     */
    @Override
    public TriggerResult onElement(Tuple2<EventBean2, Integer> element, long timestamp, TimeWindow window, TriggerContext ctx) throws Exception {
        // 如果窗口结束点 <= 当前的watermark , 判断,当前数据的用户形式时间id是否等于e0x, 如果是, 则触发
        if (("e0x".equals(element.f0.getEventId())) || (window.maxTimestamp() <= ctx.getCurrentWatermark())) {
            // if the watermark is already past the window fire immediately
            return TriggerResult.FIRE;
        } else {
            // 注册定时器的 的触发时间为窗口的最大时间
    ...
    /**
     * 当处理时间定时器触发(窗口结束时间)到达了, 检查是否满足触发条件
     */
    @Override
    ...
    /**
     * 当事件时间定时器的触发时间(窗口结束点时间)到达了, 检查是否满足触发条件
     * 本方法是定时器在调用
     */
    @Override
...
/**
 * 每次触发计算时, 不要包含其中的e0x这条数据
 * 拷贝
 * @see org.apache.flink.streaming.api.windowing.evictors.TimeEvictor
 */
class MyTimeEvictor implements Evictor<Object, TimeWindow> {
    ...
    public void evictAfter(
        Iterable<TimestampedValue<Object>> elements, int size, TimeWindow window, Evictor.EvictorContext ctx) {
        if (doEvictAfter) {
            evict(elements, size, ctx);
        }
    }
